...
bool RandomXMiner::initialize() {
    ...
    if (m_config.huge_pages) {
        flags |= RANDOMX_FLAG_HUGE_PAGES;
    }
    if (m_config.numa_aware) {
        flags |= RANDOMX_FLAG_NUMA;
    }
    
    // Configure RandomX dataset
    randomx_dataset_params dataset_params;
    randomx_init_dataset_params(&dataset_params);
    dataset_params.thread_count = m_config.init_threads;
    
    // Pre-allocate dataset if enough memory
    if (m_config.scratchpad_size > 0) {
        dataset_params.scratchpad_size = m_config.scratchpad_size;
    }
...
bool RandomXMiner::initialize_randomx(const std::string& seed_hash) {
    ...
    // Allocate cache
    m_rx_cache = randomx_alloc_cache(static_cast<randomx_flags>(RANDOMX_FLAG_HUGE_PAGES));
    if (!m_rx_cache) {
    ...
    m_rx_dataset = randomx_alloc_dataset(RANDOMX_FLAG_HUGE_PAGES);
    if (!m_rx_dataset) {
    ...
    randomx_init_dataset(m_rx_dataset, m_rx_cache, 0, num_dataset_items, m_config.init_threads);
    
    // Create VM for each mining thread
    for (uint32_t i = 0; i < m_config.num_threads; ++i) {
        randomx_vm* vm = randomx_create_vm(
            static_cast<randomx_flags>(RANDOMX_FLAG_HUGE_PAGES | RANDOMX_FLAG_JIT),
            m_rx_cache,
            m_rx_dataset
        );
...
uint64_t RandomXMiner::target_to_uint64(const std::string& target_hex) {
    // Monero target is in little-endian format
...
bool RandomXMiner::hash_meets_target(const uint8_t* hash, uint64_t target) {
    // Interpret hash as little-endian uint64 for comparison
    ...
    while (m_running.load()) {
        ...
        for (uint32_t i = 0; i < BATCH_SIZE && !m_new_job.load(); ++i) {
            ...
            if (hash_meets_target(hash, target)) {
                // Convert nonce and hash to hex strings
...
void RandomXMiner::on_share_result(bool accepted, const std::string& response) {
    if (accepted) {
        ...
        std::cout << "Share rejected: " << response << std::endl;
    }
}

void RandomXMiner::start_mining() {
    if (!m_stratum->connect()) {
        throw std::runtime_error("Failed to connect to pool");
    }
    
    if (!m_stratum->login()) {
        throw std::runtime_error("Failed to login to pool");
    }
    ...
    // Start stratum receive loop
    m_stratum->start_receive_loop();
    
    // Wait for first job
    std::cout << "Waiting for first job..." << std::endl;
    while (m_current_job.blob.empty()) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    
    // Initialize RandomX with first seed
    if (!initialize_randomx(m_current_job.seed_hash)) {
        throw std::runtime_error("RandomX initialization failed");
    }
    ...
    std::thread hashrate_thread([this]() {
        while (m_running.load()) {
            std::this_thread::sleep_for(std::chrono::seconds(10));
            
            auto elapsed = std::chrono::steady_clock::now() - m_start_time;
            double seconds = std::chrono::duration<double>(elapsed).count();
            double hashrate = m_total_hashes.load() / seconds;
            
            std::cout << "Hashrate: " << std::fixed << std::setprecision(2) 
                     << hashrate / 1000.0 << " KH/s"
                     << " | Accepted: " << m_accepted_shares.load()
                     << " | Rejected: " << m_rejected_shares.load()
                     << std::endl;
        }
    });
